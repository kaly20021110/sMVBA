		...
		if _, ok := c.HaltFlags[p.Epoch]; ok {
			if leader := c.Elector.Leader(p.Epoch, p.Round); leader == p.Author {
				if ok := c.verifyConsensusBlock(p.B); !ok {
					logger.Error.Printf("handle proposal and want to commit but error and try to retriver Author %d Epoch %d lenof Reference %d\n", p.Author, p.Epoch, len(p.B.PayLoads))
				} else {
...
func (c *Core) handleOutput(epoch int64, blockHash crypto.Digest) error {
	...
	if b, err := c.getConsensusBlock(blockHash); err != nil {
		...
		if ok := c.verifyConsensusBlock(b); !ok {
			logger.Error.Printf("handle proposal and want to commit but error and try to retriver Author %d Epoch %d lenof Reference %d\n", b.Proposer, epoch, len(b.PayLoads))
		} else {
			c.Commitor.Commit(b)
		}
	} else {
...
func (c *Core) handleLoopBack(blockhash crypto.Digest) error {
	if block, err := c.getConsensusBlock(blockhash); err != nil {
		...
		proposal, _ := NewSPBProposal(block.Proposer, block, block.Epoch, 0, SPB_ONE_PHASE, c.SigService)
		//ensure all block is received
		if proposal.Phase == SPB_ONE_PHASE {
			if _, ok := c.HaltFlags[proposal.Epoch]; ok {
				if leader := c.Elector.Leader(proposal.Epoch, proposal.Round); leader == proposal.Author {
					if ok := c.verifyConsensusBlock(proposal.B); !ok {
						logger.Error.Printf("handle proposal and want to commit but error and try to retriver Author %d Epoch %d lenof Reference %d\n", proposal.Author, proposal.Epoch, len(proposal.B.PayLoads))
					} else {
						c.Commitor.Commit(proposal.B)
					}
				}
			}
		}
		if block.Epoch >= c.Epoch { //如果已经进入下一轮就没必要再处理这个方案了
			go c.getSpbInstance(proposal.Epoch, proposal.Round, proposal.Author).processProposal(proposal)
		}
	}
	return nil
}
